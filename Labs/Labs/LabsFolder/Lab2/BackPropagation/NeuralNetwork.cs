...
namespace NeuralNetwork
{
    ...
    // Delta-Bar-Delta - видозмінена мережа BackPropagation, в якій для кожної ваги визначений власний коефіцієнт швидкості навчання
    public class DeltaBarDelta
    {
        ...
        private double[][][] learningRates; // масив швидкостей навчання для зміни ваг

        private double[][][] previousPhiDelta; // масив дельт похибок попередніх обчислень
        private double[][][] previousPhiDeltaSecond; // масив другої функції дельт похибок попередніх обчислень

        private double initialLearningRate; // почтакова швидкість навчання нейромережі
        ...
        #region Конструктор
        public DeltaBarDelta(string path, int i)
        {
            Load(path, 1);
        }

        public DeltaBarDelta(int[] layerSizes, TransferFunction[] TransferFunctions, double initialLearningRate_)
        {
            ...
            previousPhiDelta = new double[layerCount][][];
            previousPhiDeltaSecond = new double[layerCount][][];

            learningRates = new double[layerCount][][];
            ...
            for (int l = 0; l<layerCount; l++)
            {
                ...
                previousPhiDelta[l] = new double[l == 0 ? inputSize : layerSize[l - 1]][];
                previousPhiDeltaSecond[l] = new double[l == 0 ? inputSize : layerSize[l - 1]][];
                learningRates[l] = new double[l == 0 ? inputSize : layerSize[l - 1]][];

                for (int i = 0; i<(l == 0 ? inputSize : layerSize[l - 1]); i++)
                {
                    weight[l][i] = new double[layerSize[l]];
                    previousWeightDelta[l][i] = new double[layerSize[l]];
                    previousPhiDelta[l][i] = new double[layerSize[l]];
                    previousPhiDeltaSecond[l][i] = new double[layerSize[l]];
                    learningRates[l][i] = new double[layerSize[l]];
                }
            }
            ...
            for (int l = 0; l < layerCount; l++)
            {
                ...
                for (int i = 0; i < (l == 0 ? inputSize : layerSize[l - 1]); i++)
                {
                    for (int j = 0; j < layerSize[l]; j++)
                    {
                        ...
                        previousPhiDelta[l][i][j] = 0.0;
                        previousPhiDeltaSecond[l][i][j] = 0.0;
                        learningRates[l][i][j] =  initialLearningRate;
        ...
        public DeltaBarDelta(string xml)
        {
            //loaded = false;
        ...
        // Функція для обчислення дельти для зміни швидкості навчання для мережі Delta-Bar-Delta
        private double CalculateLearningRateDelta(double previousPhiDeltaSecond, double phiDelta, double kCoefficient, double bCoefficient, double learningRate)
        {
            if (previousPhiDeltaSecond * phiDelta > 0)
                return kCoefficient;
            else
            {
                if (previousPhiDeltaSecond * phiDelta < 0)
                    return -bCoefficient * learningRate;
                else
                    return 0.0;
            }
        ...
        public void TrainNetwork(double[][] patterns, double[][] answers, double min_error, double kCoef, double bCoef, double eCoef, double Momentum)
        {
            ...
            do
            {
                ...
                for (int k = 0; k < amountOfPatterns; k++)
                {
                    ...
                    error += Train(ref TrainingSet[index].trainVector, ref TrainingSet[index].answer, kCoef, bCoef, eCoef, Momentum);
        ...
        public void Save(string filepath)
        {
            ...
            writer.WriteAttributeString("Type", "DeltaBarDelta");
        ...
        public void Load(string path, int abc)
        {
             if (path == null)
             {
                 loaded = false;
    ...
    public static class Normalize
    {
        ...
        public static double[][] FormAnswersDeltaBarDelta(double[][] answers)
        {
            double[][] returnAnswer = new double[answers.Length][];
            for (int i = 0; i < answers.Length; i++)
                returnAnswer[i] = new double[1];
